...
impl PackageMetadataFslabsCiPublishCargo {
    pub async fn check(&mut self, name: String, version: String, cargo: &Cargo) -> anyhow::Result<()> {
        ...
        let registry_name = registries.get(0).unwrap().clone();
        log::debug!("CARGO: checking if version {} of {} already exists for registry {}", version, name, registry_name);
        self.publish = cargo.check_crate_exists(registry_name, name, version).await?;
...
impl Cargo {
    pub fn new(crates_io_token: Option<String>) -> anyhow::Result<Self> {
        ...
        registries.insert("default".to_string(), CargoRegistry::new(CARGO_DEFAULT_API_URL.to_string(), crates_io_token));
        Ok(Self {
    ...
    pub fn add_registry(&mut self, name: String, crate_url: String, token: Option<String>) -> anyhow::Result<()> {
        let reg = CargoRegistry::new(crate_url, token);
        self.registries.insert(name, reg);
        Ok(())
    }

    pub async fn check_crate_exists(&self, registry_name: String, name: String, version: String) -> anyhow::Result<bool> {
        let registry = self.registries.get(&registry_name).ok_or_else(|| anyhow::anyhow!("unknown registry"))?;
        let url: Uri = format!("{}{}", registry.crate_url, name).parse()?;

        let user_agent = registry.token.clone().unwrap_or_else(|| "fslabsci".to_string());
        ...
        let res = self.client.request(req).await.with_context(|| "Could not fetch from the crates registry")?;
        ...
        let package: Option<CargoPackage> = match serde_json::from_str::<CargoSearchResult>(body_str.as_ref()) {
            Ok(search_result) => {
                match (search_result.crates, search_result.single_crate) {
                    (Some(crates), None) => crates.into_iter().find(|c| c.name == name).map(|c| c.clone()),
                    (_, Some(single_crate)) => {
...
mod tests {
    ...
    async fn cargo_test(
        ...
        expected_result: bool, expected_error: bool,
        ...
        if let (Some(user_agent), Some(mock_status), Some(mock_body)) = (mock_user_agent, mock_status, mock_body) {
            let mock_server = MockServer::start().await;
            let prefix = "krates/by-name/".to_string();
            Mock::given(method("GET"))
                .and(path(format!("{}{}", prefix, package_name)))
                .and(header("User-Agent", user_agent.clone()))
                .respond_with(ResponseTemplate::new(mock_status).set_body_raw(mock_body, "application/json"))
                .mount(&mock_server)
                .await;
            let mock_server_uri = mock_server.uri();
            registry = "private".to_string();
            cargo.add_registry(registry.clone(), format!("{}/{}", mock_server_uri, prefix), registry_user_agent).expect("could not add private registry");
        }

        let result = cargo.check_crate_exists(registry, package_name, package_version).await;
        match result {
            Ok(exists) => {
    ...
    async fn cargo_existing_crate_and_version() {
        cargo_test("rand".to_string(), "0.8.4".to_string(), None, true, false, None, None, None).await;
    }

    #[tokio::test]
    async fn cargo_existing_crate_and_not_version() {
        cargo_test("rand".to_string(), "99.99.99".to_string(), None, false, false, None, None, None).await;
    }

    #[tokio::test]
    async fn cargo_existing_crate_and_version_private_reg() {
        cargo_test("hub_app".to_string(), "0.2.0".to_string(), Some("my_registry my_token".to_string()), true, false, Some("my_registry my_token".to_string()), Some(200), Some(EXISTING_PACKAGE_DATA.to_string())).await;
    }

    #[tokio::test]
    async fn cargo_existing_crate_and_not_version_private_reg() {
        cargo_test("hub_app".to_string(), "99.99.99".to_string(), Some("my_registry my_token".to_string()), false, false, Some("my_registry my_token".to_string()), Some(200), Some(EXISTING_PACKAGE_DATA.to_string())).await;
    }
    //
    // #[tokio::test]
